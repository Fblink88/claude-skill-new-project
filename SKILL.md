...
6. **Modo de captura del contexto inicial** — distinto de los ejes anteriores: esto es *cómo* se recoge la información, no *qué tan a fondo* (1), *cómo se explica* (2) ni *en qué idioma* (3). "¿Prefieres contarme de una vez todo lo que tengas pensado o escrito sobre el proyecto, en tus palabras — y de ahí yo saco lo que necesito y te pregunto solo lo que falte — o prefieres que te vaya preguntando paso a paso, bloque por bloque?"
   - **Volcado libre** — describe el proyecto en un solo texto largo, sin que las preguntas acoten la respuesta. Si ya lo tienes escrito en un documento (no solo en la cabeza), puedes pegarlo directo o guardarlo en `DOC/insumos/` y decírmelo (ver Transversal) — mismo tratamiento.
   - **Paso a paso** (default si no se pregunta) — flujo guiado bloque por bloque, como el resto de esta skill.

   No aplica si el alcance elegido en (1) fue "sin guía de decisiones" — ahí no hay bloques que recorrer.

   **Si eligió "volcado libre":** pide el texto completo en un solo mensaje, sin interrumpir con preguntas mientras lo escribe. Con el texto en mano, aplica el mismo mecanismo de mapeo descrito más abajo en "Si es existente → Cómo se hace el mapeo" (clasificación de los 13 bloques en Resuelto/Parcial/Abierto/Cambiar, resumen de confirmación, excepción del bloque 5) — no se limita a los bloques 2 y 3, cualquier bloque que el texto ya cubra (stack, equipo, etc.) se marca resuelto igual. Además del resumen de confirmación, para lo que quedó parcial por ser ambiguo o muy breve (no por ausencia del tema), señálalo puntual antes de seguir — no basta con la clasificación general: "Sobre [tema], escribiste '[resumen corto]' — ¿eso es todo, o hay más que quieras agregar?" Así se pule el contexto ya capturado en vez de darlo por bueno solo porque es largo.
...
- `DOC/avances.md`, `DOC/errores-y-arreglos.md`, `DOC/cambios-del-plan.md` — bitácoras para la fase de implementación (se empiezan a usar ahí, no durante el descubrimiento). Cómo se dividen entre archivos (por persona, por microservicio/pista, ambas, o un solo archivo) se pregunta explícito al cerrar el kickoff, no se asume en silencio — ver Cierre del kickoff, punto 5.
...
- `DOC/modelo-datos.md` (MER: entidades, relaciones, nivel de normalización objetivo, y un diagrama Mermaid `erDiagram`) y `DOC/glosario.md` (vocabulario del dominio, un término = un significado) — del bloque 4, persistidos como documento, no solo en la conversación.
...
- `DOC/plan-implementacion.md` — desglose ejecutable de tareas del cierre del kickoff, solo cuando ese desglose queda dividido por microservicio o por frontend/backend separado (bloque 6) y hay más de una persona (bloque 10) — evita que `DOC/plan.md` crezca sin control con varias pistas paralelas corriendo a la vez. Si no hay esa división (monolito integrado, o una sola persona), el desglose se mantiene dentro de `DOC/plan.md` como el resto. Ver Cierre del kickoff para el detalle de cómo se organiza.
...
- `DOC/insumos/` — única carpeta (no archivo) de esta lista: material externo que el usuario ya tenga (un plan en `.md`, un Excel, un PDF, notas) que deba considerarse durante el proceso. Es carpeta y no archivo porque puede haber cualquier cantidad de archivos sueltos de formatos distintos — el resto de `DOC/` se mantiene flat porque la mayoría de estos documentos ni siquiera llegan a crearse en un proyecto chico (se disparan bloque por bloque, no todos existen siempre). Preguntar al crear la estructura: "¿Tienes algún documento externo ya armado que deba tener en cuenta? Puedes dejarlo en `DOC/insumos/` o decirme la ruta." Revisar esta carpeta al inicio y cada vez que se retome el proyecto. Si el formato no se puede leer directo (ej. un Excel sin exportar), avisar y pedir que se exporte a texto/CSV en vez de asumir su contenido. **Después de leerlo, no usarlo tal cual sin revisión:** preguntar "Leí [nombre del documento] — ¿quieres que lo revisemos juntos antes de seguir (aclarar ambigüedades, completar lo que falte, dejarlo ordenado), o prefieres que lo use tal como está?" Si acepta pulirlo, ofrecer un resumen de lo entendido y señalar puntos ambiguos o contradictorios antes de darlo por incorporado al proceso — un insumo externo mal interpretado contamina todos los bloques que dependen de él.
...
- **Mixta** — combina más de una de las anteriores (ej. web para administración + app móvil para usuarios finales, o web + API pública). Especifica cuáles, y para qué sirve cada una — no asumir que todas comparten el mismo alcance.
...
**Bloqueantes — prosa:**
1. "¿Cuál es la versión mínima que vas a construir primero — el MVP — para que el proyecto sea útil desde el día uno? Todavía no existe nada, esto es lo que planeamos construir primero." (ej. "que pueda registrar un gasto y verlo en una lista, nada de reportes ni gráficos todavía")
...
**Descripción por funcionalidad — siempre, para cada funcionalidad marcada (preguntas 3 y 4):** no basta con el nombre genérico de la opción marcada — pedir una línea concreta de qué hace específicamente en este proyecto (ej. no "reportes", sino "reporte mensual de gastos por categoría, exportable a PDF"). Guardar en `DOC/plan.md`; esta descripción es la base de las épicas e historias de usuario de más abajo.

**Completo — reglas de negocio, prosa:**
5. "¿Hay reglas de negocio específicas de este proyecto? No respondas solo sí/no — repasa estas categorías antes de descartarlas: **quién puede hacer qué** (¿alguien necesita aprobar algo antes de que quede confirmado? ¿hay acciones que no se pueden deshacer?) · **validaciones o límites** (ej. un monto máximo, un cupo, una fecha límite, un campo obligatorio bajo cierta condición) · **cálculos o fórmulas propias del negocio** (ej. cómo se calcula un descuento, un puntaje, un total) · **excepciones** (casos especiales que rompen la regla general — ¿qué pasa distinto ahí?)." Guardar lo que salga en `DOC/plan.md`; si sale algo con formato condicional claro ("si X entonces Y"), es candidato directo a EARS-lite (pregunta de abajo) sin tener que reformularlo después. Esta captura inicial no es la última palabra — se revisa de nuevo más abajo, contra las historias de usuario ya concretas.

**Completo — épicas**, solo si el tamaño (bloque 2) no fue "uso personal o uso ocasional" (misma compuerta que las historias de usuario formales, pregunta 6):
5.5. "¿Agrupamos las funcionalidades en épicas — grandes bloques temáticos (ej. 'Gestión de gastos', 'Reportes', 'Administración de usuarios') — antes de armar las historias de usuario? Ayuda a organizar el trabajo sin importar la metodología que uses después (Scrum, Kanban o ninguna)." Si acepta, proponer una agrupación razonada a partir de las funcionalidades y descripciones ya recogidas (preguntas 3-4) y confirmar con el usuario — la cantidad de épicas depende del proyecto, no hay un número fijo (un proyecto chico puede tener 2-3, uno más grande 5 o más). Guardar en `DOC/plan.md`, junto con las historias de usuario.

**Completo — historias de usuario formales**, solo si el tamaño (bloque 2) no fue "uso personal o uso ocasional":
6. "¿Quieres que transformemos la lista de funcionalidades en historias de usuario formales (funcionalidad → quién la usa → para qué), o te basta con la lista?" Si se armaron épicas (pregunta 5.5), cada historia queda asociada a su épica, y se generan épica por épica —arma primero las de una épica, refínala (ver abajo), y recién pasa a la siguiente— en vez de todas de un tirón. No hay un tope artificial de cantidad: un proyecto con varias épicas bien pobladas puede terminar con historias en las decenas, no limitarse a 10-15 solo porque cubre lo obvio.

**Refinamiento de historias de usuario** (si aceptó la pregunta 6): generar un primer lote no es el final del paso — un lote inicial suele quedarse corto. Antes de darlo por cerrado —por cada épica si se armaron, o sobre el total si no— revisar con el usuario: "Estas son las historias que salieron de [la épica X / las funcionalidades que ya definimos]: [lista]. ¿Cubren todos los casos que te importan, o falta alguna? Piensa en: roles distintos que todavía no tienen su propia historia, casos límite (qué pasa si algo falla, si no hay datos, si se repite una acción), y excepciones de la pregunta 5 que deberían tener su propia historia." Ofrecer explícitamente agregar más si faltan — no asumir que el primer lote alcanza solo porque cubre las funcionalidades obvias.

**Cierre del refinamiento — reglas de negocio revisitadas.** Con todas las historias ya armadas (todas las épicas, o el lote único si no hubo épicas), una pasada final: "¿Alguna de estas historias necesita una regla de negocio que no habíamos anotado en la pregunta 5 (un límite, una validación, un cálculo, una excepción)?" Suele ser más fácil detectar reglas mirando casos concretos que en abstracto — agregar lo que salga a lo ya registrado en `DOC/plan.md`, no reemplazarlo.
...
Lee `references/modelo-datos.md` antes de la pregunta 5 — tiene el detalle de las formas normales (por qué 3FN es el default y cuándo 4FN aporta algo real) y la guía para armar el diagrama `erDiagram`.

**Alcance:** este bloque llega hasta el modelo conceptual (entidades y relaciones) — no diseña la base de datos física. Campos exactos, tipos de dato y claves se resuelven después, al implementar. El nivel de normalización objetivo (pregunta 5) es la excepción: se fija ahora como preferencia — igual que estable-vs-nuevo en el bloque 7 — para aplicarla cuando se diseñe la base de datos física, no implica diseñar las tablas ahora.
...
5. **Nivel de normalización objetivo.** "¿El modelo apunta a Tercera Forma Normal (3FN) — el estándar para la gran mayoría de proyectos, evita datos duplicados sin volverse demasiado complejo — o hay algún caso en el dominio que ya suene a Cuarta Forma Normal (4FN)?" Proponer 3FN como default razonado; sugerir evaluar 4FN solo si alguna entidad ya descrita tiene dos conjuntos de datos independientes entre sí que hoy vivirían juntos en la misma tabla (ver `references/modelo-datos.md` para el ejemplo). Ante la duda, 3FN. Documentar la elección y el motivo en `DOC/modelo-datos.md` — se aplica más adelante al diseñar la base de datos física, no cambia el modelo conceptual de este bloque.

**Persistencia, diagrama y regla de mantenimiento:** el resultado de este bloque se guarda en `DOC/modelo-datos.md` (MER) — no solo en la conversación, y no solo como texto. Además de la lista de entidades, atributos conocidos y relaciones, generar un diagrama Mermaid `erDiagram` con las entidades, sus atributos ya conocidos (los que el usuario mencionó espontáneamente — no inventar campos que no se han hablado) y las relaciones con su cardinalidad (1:1, 1:N, N:M) — ver `references/modelo-datos.md` para la sintaxis. Mismo mecanismo que el diagrama C4 del bloque 6 en `DOC/arquitectura.md`. Regla permanente para `CLAUDE.md`: cada vez que una migración cree o modifique una tabla, actualizar tanto el texto como el diagrama de `DOC/modelo-datos.md` en el mismo cambio — nunca después ni acumulando varias migraciones sin reflejarlas. Un modelo desincronizado de la base real deja de servir.
...
**Patrones adicionales, por trigger — no se preguntan de oficio, se ofrecen solo si aparece la señal correspondiente** (detalle completo, costos reales y cuándo sí/no en `references/arquitectura.md`):
- **Serverless / FaaS.** Trigger: tamaño personal o pequeño (bloque 2) + tráfico esperado esporádico o impredecible, o el usuario dice explícitamente que no quiere administrar un servidor. Se ofrece como alternativa a "monolito simple" en la pregunta 2, no como sinónimo de "distribuido" a gran escala — un proyecto chico y sin equipo puede ser 100% serverless. Costo real a mencionar: cold starts (demora extra en la primera invocación tras estar inactivo), límite de tiempo de ejecución por invocación, y vendor lock-in más fuerte que un monolito tradicional (se retoma en el bloque 9).
- **Event-Driven.** No es alternativa a monolito/distribuido, es ortogonal — un monolito modular puede tener un event bus interno, y microservicios casi siempre se comunican así entre ellos. Trigger: bloque 3 marcó "procesos que tardan o corren en segundo plano", o el dominio (bloque 4) tiene una acción que dispara varias reacciones desacopladas (ej. "pedido pagado" → actualizar inventario + enviar email + generar factura). Si aparece, preguntar: "¿estas acciones necesitan reaccionar unas a otras de forma desacoplada, o alcanza con llamarlas directo en el mismo flujo de código?" En proyectos chicos, la opción directa (sin cola de eventos) suele alcanzar — no proponer infraestructura de eventos solo porque existe un flujo de reacciones.
- **Microkernel (patrón de plug-ins).** Nicho, nunca se ofrece de oficio. Trigger: el usuario menciona explícitamente que quiere que terceros (o módulos internos) extiendan el sistema sin tocar el núcleo (ej. "quiero que se puedan agregar plugins", "como WordPress pero para X"). Si aparece, explicar el patrón y confirmar que es lo que busca antes de aplicarlo.
- **SOA (arquitectura orientada a servicios).** Nicho, casi siempre reemplazado por microservicios en un proyecto nuevo desde cero. Trigger: el usuario menciona que debe integrarse con sistemas empresariales o legacy ya existentes que no puede modificar. Si aparece, explicar la diferencia con microservicios y por qué rara vez conviene elegirlo como arquitectura de base — se justifica solo por la necesidad de integración.
...
1.1. **Solo si la pregunta 1 dio más de una persona:** "¿Cada quien va a trabajar en todo (full-stack), o hay especialización por persona (ej. alguien dedicado a frontend, otro a backend/infra)? Cuéntame brevemente quién se encarga de qué." Se guarda para repartir tareas más adelante, en el cierre del kickoff (desglose en tareas) — no se vuelve a preguntar ahí.
...
2. **Desglose en tareas.** Convertir el plan (bloques 2-11) en unidades de trabajo concretas y secuenciadas, con sus dependencias.

   **2.1 Eje de división — según arquitectura (bloque 6):**
   - Arquitectura distribuida/microservicios → un desglose separado por cada microservicio (cada uno con sus propias tareas, etapas y configuración — no se mezclan en una lista única).
   - Frontend y backend separados como proyectos (bloque 6, pregunta 3) → dos pistas separadas, Plan Frontend y Plan Backend.
   - Monolito integrado → un desglose único, sin división — como hasta ahora.

   Si queda dividido (microservicio o front/back) **y** hay más de una persona (bloque 10, pregunta 1): el desglose se guarda en `DOC/plan-implementacion.md` en vez de dentro de `DOC/plan.md` (ver Transversal). Si no queda dividido, o es una sola persona, se mantiene dentro de `DOC/plan.md`.

   **2.2 Contenido de cada pista — según metodología (bloque 10, pregunta 1.5):** dentro de cada pista que haya resultado de 2.1 (o del desglose único si no hubo división), convertir el plan en tareas: si en el bloque 3 se armaron historias de usuario formales, cada una se parte en tareas; si no, se arma la lista directamente desde las funcionalidades. Si además se armaron épicas (bloque 3, pregunta 5.5), agrupar las tareas por épica antes de ordenarlas por dependencia — da una estructura intermedia entre "toda la lista de tareas" y "una tarea suelta", útil sobre todo cuando el desglose no quedó dividido por pista (2.1). Después:
   - **Scrum** → ver punto 3 (Product Goal + sprints).
   - **Kanban** → tablero clásico de columnas (Por hacer / En curso / Hecho), flujo continuo — no se agrupa en fases ni hitos temporales.
   - **Ninguna metodología formal** → lista de tareas ordenada por dependencia, sin estructura adicional.

   **2.3 Reparto por persona** — solo si hay más de una persona: usar la especialización declarada en el bloque 10 (pregunta 1.1) para asignar cada tarea a una persona concreta dentro de su pista/columna/sprint, no dejar tareas sin dueño. Ej.: "Plan Frontend, columna 'Por hacer': [Persona A] — pantalla de login. Plan Backend, columna 'Por hacer': [Persona B] — endpoint de autenticación."
3. **Si se eligió Scrum (bloque 10):** antes de organizar los sprints, preguntar explícito: "¿Cuál es el objetivo de fondo de todo el producto — el Product Goal? Es distinto del objetivo del bloque 2: ese era el problema que resuelve, este es el norte que van a perseguir todos los sprints juntos, se define una sola vez para todo el producto, sin importar si el desglose (2.1) queda dividido en pistas." Con eso ya definido: si 2.1 generó más de una pista, cada pista arma su propia secuencia de sprints con su propio **Sprint Goal**, todos aportando al mismo Product Goal; si no hubo división, una sola secuencia de sprints. La Definition of Done general (bloque 12, cuando se active) puede tener criterios adicionales por sprint — se agregan ahí, no se duplican.
...
5. **Cómo dividir las bitácoras de implementación** (`DOC/avances.md`, `DOC/errores-y-arreglos.md`, `DOC/cambios-del-plan.md`, ver Transversal): con la arquitectura (bloque 6) y el equipo (bloque 10) ya resueltos, preguntar explícito en vez de asumir en silencio — "¿Cómo prefieres llevar estas bitácoras durante la implementación?"
   - **Un solo archivo compartido** — todos escriben en el mismo.
   - **Por persona** — un archivo por quien trabaja (ej. `DOC/avances-[nombre].md`).
   - **Por microservicio o pista** — un archivo por microservicio, o por Frontend/Backend si quedaron separados (ej. `DOC/avances-pagos.md`, `DOC/avances-frontend.md`) — misma división que el punto 2.1 de arriba.
   - **Por microservicio/pista y persona** — combinado, un archivo por persona dentro de cada microservicio/pista — solo aporta con equipos grandes, varias personas trabajando en el mismo servicio.

   Proponer un default razonado y dejar que el usuario confirme o ajuste: si 2.1 generó pistas (microservicio o front/back) → proponer "por microservicio/pista"; si no hubo división pero hay más de una persona → proponer "por persona"; si es una sola persona → proponer "un solo archivo compartido". Aplica a las tres bitácoras por igual — no dividir cada una con un criterio distinto.
