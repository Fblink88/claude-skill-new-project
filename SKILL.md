...
**Refinamiento de historias de usuario** (si aceptó la pregunta 6): generar un primer lote no es el final del paso — un lote inicial suele quedarse corto. Antes de darlo por cerrado, revisar con el usuario: "Estas son las historias que salieron de las funcionalidades que ya definimos: [lista]. ¿Cubren todos los casos que te importan, o falta alguna? Piensa en: roles distintos que todavía no tienen su propia historia, casos límite (qué pasa si algo falla, si no hay datos, si se repite una acción), y excepciones de la pregunta 5 que deberían tener su propia historia." Ofrecer explícitamente agregar más si faltan — no asumir que el primer lote alcanza solo porque cubre las funcionalidades obvias.
...
3. **Si se eligió Scrum (bloque 10):** antes de organizar los sprints, preguntar explícito: "¿Cuál es el objetivo de fondo de todo el producto — el Product Goal? Es distinto del objetivo del bloque 2: ese era el problema que resuelve, este es el norte que van a perseguir todos los sprints juntos, se define una sola vez." Con eso ya definido, organizar el desglose en sprints — cada sprint tiene su propio **Sprint Goal** (objetivo puntual de ese sprint) que aporta al Product Goal. La Definition of Done general (bloque 12, cuando se active) puede tener criterios adicionales por sprint — se agregan ahí, no se duplican.
