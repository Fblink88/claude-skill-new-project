...
- **RLS (Row-Level Security)** — no es solo para multi-tenancy (bloque 8, ver `references/auth-y-multitenancy.md`): aplica como buena práctica general en bases de datos que lo soportan (ej. PostgreSQL/Supabase), con política "deny by default" — nadie ve nada salvo que una política lo permita explícitamente, incluso en proyectos de un solo tenant.
