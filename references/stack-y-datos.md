...
- Según plataforma (bloque 2): web/PWA → depende de la respuesta de SEO (pregunta 1 del bloque 7): sin necesidad de indexación (herramienta interna, dashboard logueado, PWA privada) → framework SPA liviano (React+Vite, Vue+Vite, SvelteKit) + plugin de PWA (ej. `vite-plugin-pwa`) — bundle más chico, dev server más rápido, service worker más simple de razonar porque no convive con SSR/ISR; con necesidad real de SEO/indexación (contenido público que debe aparecer en buscadores, previews de redes) → ahí se justifica un framework con SSR (ej. Next.js, Nuxt) pese al peso extra. No default a SSR "porque es lo más completo" sin este criterio — mismo principio que evitar moda sin razón (bloque 6). Angular es otra opción PWA madura pero conviene reservarla para proyectos grandes (trae RxJS, DI, Zone.js) — no proponerla por default en proyectos chicos/medianos. Móvil → nativo o multiplataforma según si se necesita rendimiento nativo o rapidez de desarrollo; escritorio → framework multiplataforma salvo integración profunda con el sistema operativo; CLI → lenguaje con buen soporte de librerías de terminal y distribución simple.
