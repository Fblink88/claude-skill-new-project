...
3. **Alcance** — MVP, qué queda deliberadamente fuera, funcionalidades (incluye pagos/suscripciones tipo SaaS, notificaciones automáticas, procesos en segundo plano), reglas de negocio, agrupadas en épicas con sus historias de usuario cuando el proyecto lo amerita (sin tope artificial de cantidad), y para quien necesita trazabilidad real (ej. auditoría legal) la opción de formular requisitos críticos en formato testeable estilo EARS.
4. **Modelo del dominio** — entidades y relaciones (sin llegar al diseño físico de base de datos: eso es implementación), con su diagrama `erDiagram` en Mermaid y el nivel de normalización objetivo (3FN por default, 4FN solo si el caso lo amerita), más un glosario de vocabulario del dominio para que un mismo concepto no termine con varios nombres distintos a lo largo del proyecto.
...
6. **Arquitectura y paradigma** — si hace falta servidor o no; monolito, monolito modular o arquitectura distribuida (con la heurística real detrás, no la moda de turno); separación frontend/backend; patrones adicionales cuando el caso concreto los justifica (serverless/FaaS, event-driven, microkernel, SOA); y un diagrama de arquitectura estilo C4 (niveles de contexto y contenedores).
...
10. **Git, ambientes, CI/CD y convenciones** — dónde vive el repositorio, formato de commits (y si mencionan uso de IA o no), cantidad de ambientes (local/staging/producción), si el equipo se especializa por área (frontend/backend/infra) para repartir tareas después, metodología de trabajo si se quiere una formal (Scrum, Kanban), y convenciones de código (nombres, nivel de comentarios).
...
**Cierre del kickoff, antes de implementar:** el plan se convierte en un desglose de tareas concretas — dividido por microservicio o por frontend/backend si la arquitectura quedó separada así, organizado en sprints (Scrum) o en un tablero de columnas (Kanban), y repartido por persona si el equipo se especializó por área —, se define cómo se dividen las bitácoras de avance para la implementación, y se arma un registro de riesgos agregando las señales ya levantadas en los bloques anteriores.
...
Documentación viva (`DOC/plan.md`, `DOC/decisiones.md`, `DOC/modelo-datos.md`, `DOC/glosario.md`, `DOC/plan-implementacion.md`, `DOC/riesgos.md`, entre otros) y guardrails de IA (`CLAUDE.md`) corren en paralelo desde el bloque 1, no son un bloque aparte.
